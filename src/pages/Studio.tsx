...
export default function Studio() {
  ...
  return (
    <div className="w-full pt-24 pb-20 overflow-x-hidden">
      <section className="container mx-auto px-6 md:px-12 mb-20 md:mb-32 relative">
        <motion.div
          initial="hidden"
          animate="visible"
        ...
        >
          <motion.div variants={fadeUp} className="mb-6 inline-flex items-center space-x-2">
            ...
            <span className="text-[10px] md:text-xs font-medium tracking-widest text-primary uppercase">Lumeniax Studio</span>
          </motion.div>
          <motion.h1 variants={fadeUp} className="text-4xl sm:text-5xl md:text-7xl font-serif font-medium leading-[1.1] mb-8 text-foreground break-words">
            L'art de la <span className="italic text-primary">transformation</span> digitale.
          </motion.h1>
          <motion.p variants={fadeUp} className="text-base md:text-lg text-muted-foreground mb-12 max-w-2xl leading-relaxed">
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
            <Link href="/studio/services">
              <Button size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 h-14 px-8 text-xs md:text-sm uppercase tracking-widest w-full sm:w-auto">
                Nos Services <ArrowRight className="ml-3 w-4 h-4" />
              </Button>
            </Link>
            <Link href="/studio/portfolio">
              <Button size="lg" variant="outline" className="border-border h-14 px-8 text-xs md:text-sm uppercase tracking-widest hover:border-primary/50 transition-colors w-full sm:w-auto">
                Voir le Portfolio
              </Button>
            </Link>
          </motion.div>
      ...
      {/* Stats Section */}
      <section className="bg-card border-y border-border/50 py-16 md:py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          <motion.div 
            ...
            viewport={{ once: true }}
            variants={staggerContainer}
            className="grid grid-cols-2 md:grid-cols-4 gap-y-10 gap-x-4 md:divide-x md:divide-border/50"
          >
            {stats.map((stat, i) => (
              <motion.div key={i} variants={fadeUp} className="text-center px-2 md:px-4">
                <div className="text-3xl md:text-5xl font-serif text-primary mb-2">{stat.value}</div>
                <div className="text-[10px] md:text-sm font-medium uppercase tracking-wider text-muted-foreground">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      {/* Testimonials */}
      <section className="bg-card py-20 md:py-32 border-y border-border/50">
        <div className="container mx-auto px-6 md:px-12">
          <motion.div
            initial="hidden"
            whileInView="visible"
            ...
            {testimonials.map((test, i) => (
              ...
              >
                ...
                <div className="mb-8">
                  ...
                  <p className="text-base md:text-lg leading-relaxed font-serif italic text-foreground/90">"{test.text}"</p>
                </div>
                <div>
